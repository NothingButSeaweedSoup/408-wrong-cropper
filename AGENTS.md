...
- **选错题页先选年份**：年份/科目都是**下拉**，进入时**不预选**（两项都显示「请选择」），
  选了年份才 `GET /api/questions?year=X` 拉那一年——**没有「全部年份」**，题目多的时候不再一次性拉全库；
  拉过的年份在 `state.cache` 里留一份，切来切去不重复请求。科目留空 = 不过滤（不选就是全部），
  所以"一年真题都没导入"时提示也变了（见 `renderQuestions()` 的空状态文案）。
- **底栏「已选 n 题」是按钮**（`#selInfo` → `#selPanel`）：点开是已选清单，一行一条纯文本
  `2009年第10题-数据结构-选择题`（科目用全称 `ZC.SUBJECT_FULL`，卡片上那个「计组」短名太简略），
  每条右边 ✕ 逐条删，右上「一键删除」清空。清单每次打开都 `GET /api/questions?ids=1,2,3` 现取一次
  （勾选跨年份、题目也可能被管理员改过题号或删掉），服务端没返回的显示成"已失效，删掉吧"。
  卡片元素存在 `cards` Map 里，删一条只改样式、不重渲染图片（否则会闪一下还重新拉图）。
...
- **题目列表有进程内缓存**（`api/questions.py` 的 `_questions_cache` + `_cache_version`）：
  ...
  **任何写操作都要调 `bump_questions_cache()`**（改题号/删题/合并/切分/重裁/导入/删卷，
  ...
  `GET /api/questions?ids=1,2,3`（用户端「已选清单」用）**故意不进缓存**：清单随勾选一直在变，
  跨年份也没法用年份 key。**注意 SQLite 里 `IN ()` 是语法错误**，所以 `ids=` 解析出空列表时要直接
  返回 `[]`，别把空 IN 丢给数据库（`tests/http_smoke.py` 3.5 段覆盖了空/垃圾/不存在的 id）。
