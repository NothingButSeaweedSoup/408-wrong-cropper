...
2. **得分记录与趋势**：用户选真题年份 → 按题组填答对个数 / 综合题填得分 → 系统算分入库 → 折线图看趋势（X 轴可切「做题时间 / 真题年份」）。
...
2. **选错题**：按年份/科目筛选 → 点卡片多选 → 底部「生成 Word 错题本」→ 自动下载 `408错题本_YYYY-MM-DD_HHMM.docx`。
   （勾选状态存浏览器本地，不占服务器）
3. **得分**：
   - 点「录入成绩」：选真题年份 + 做题日期 → 选择题填**答对个数**（1-11 数据结构、12-22 计组、23-32 OS、33-40 计网，每题 2 分）→
     综合题 41-47 逐题填**得分/满分**（各年分布不同，满分可改）→ 页面实时算模块分与总分 → 保存。
   ...
   - 曲线图上方的**当前水平估计**（一个小表格，不是图）：最近三次成绩加权 —— 最近 50%、第二近 35%、第三近 15%；
     ...
     其中**总分用原始分加权**（满分 150），**模块先按各条记录当时的满分算得分率再加权**（各年综合题分布不同，
     不按固定 45 分除，避免偏差）。它始终按做题时间取最近三次，与图表 X 轴怎么切无关。
...
Cookie 登录态、用户注册登录、得分录入/编辑/删除/趋势/水平估计、错题校正、导出下载校验，跑完自动清理测试数据。
