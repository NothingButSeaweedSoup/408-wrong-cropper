...
3. **先在下拉里选这份真题的年份**，再选 PDF → **上传并自动切题**（进度条实时走 渲染 → OCR → 切题）。
   年份必须显式选：用户端按年份看题、按年份录成绩，选错会跟别的年份混在一起。
   一年传多份没问题（不同来源/批次），它们会都算这一年。
4. 页面图上拖色块上下边缘改边界（可开「相邻题边界联动」），点色块选位置后「✂ 在此处切分」；
   右侧列表可改题号/科目/题型/分值、并入上一题、删除。
5. 改完点「整卷重裁」重新出图。
6. **年份填错了也能救**：真题库每行前面有年份徽标，选中那份卷子后工具栏里有「年份」输入框，
   改完回车即搬家到那一年（只改归属，题目和图片都不动）；也可以删掉重新传。
7. 顶部切到「**试卷结构**」：切完题会自动按题目分好组（题号范围 + 每题分值，综合题的分值还会尝试
...
2. **选错题**：进来时**年份和科目都是「请选择」**（不预选任何一项），
   **先选年份**（一次只加载这一年的题，题目再多也不卡；没有「全部年份」选项，
   服务端按年份查、前端再把看过的年份缓存住）→ 科目可不选（不选就是全部）→ 点卡片多选 →
   底部「生成 Word 错题本」→ 自动下载 `408错题本_YYYY-MM-DD_HHMM.docx`。
   勾选**跨年份累计**（存浏览器本地，不占服务器），所以「2009 选几道 + 切到 2010 再选 + 一起导出」可以。
   底部的「**已选 n 题**」是个按钮：点开是已选清单（一行一条，如 `2009年第10题-数据结构-选择题`），
   每条右边 ✕ 可以删掉这一题，右上角「一键删除」清空全部。
3. **得分**：
   - 点「录入成绩」：**年份是下拉选**（列出已导入真题的年份）+ 做题日期 → 表单**按那份试卷的结构生成**
     （选择题逐组填**答对个数**，组数和题号范围取自试卷，某年 DS 只有 10 题就只显示 10 题）→
     综合题逐题填**得分/满分**（满分默认取卷面结构，可手改）→ 页面实时算模块分与总分 → 保存。
     该年份还没导入真题时退回默认结构（1-11/12-22/23-32/33-40，每题 2 分），表单上会写清楚。
     **中途换年份**：已经填的「得分」会跟着带过去，但「满分」一律换成新年份那份卷子的
     （满分是卷子的属性，不是你的录入值），所以换到哪一年看到的就是哪一年的分值。
...
- **改题属管理操作**：`GET /api/questions*` 登录即可，但 `PATCH/DELETE /api/questions/{id}` 要管理员
  （权限判断按「方法 + 路径」，不是光看前缀）。
- **题目查询支持 `?ids=1,2,3`**：用户端「已选清单」用它按 id 取题（勾选会跨年份，按年份查不到），
  `ids=` 传空或查不到都返回 `[]`（不进进程内缓存）。
